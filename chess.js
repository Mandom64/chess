...
class Board {
    ...
    */
    isValidMove(str) {
        ...
        if(to.row < 0 || to.row > board.rows || 
           to.col < 0 || to.col > board.cols) {
        ...
        switch (piece) {
            case "p":
                ...
                if(this.square[to.row][to.col] == "0") {
                    if(from.row == 1 && Math.abs(from.row - to.row) <= 2)
                        return true;
            ...
            case "P":
                ...
                if(this.square[to.row][to.col] == "0" && Math.abs(from.row - to.row) == 1)
                    return true;
                break;
            case "R":
                return true;
    ...
    if(fields[2] == "-") {
        ...
        for(i = 0; i < fields[2].length; i++) {
            switch (fields[2].charAt(i)) {
                case "k": castling.k = 1; break;
                case "q": castling.q = 1; break;
                case "K": castling.K = 1; break;
                case "Q": castling.Q = 1; break;
            }
        }
